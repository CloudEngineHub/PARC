...
DEFAULT_CHAR_FILE = "data/assets/humanoid.xml"
...
def build_misc_data(raw_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    for key in raw_data.keys() - LEGACY_FIELDS:
        if raw_data[key] is not None:
            misc_items[key] = _convert_misc_value(raw_data[key])
...
def motion_to_ms_file(
    ...
    if frames.ndim != 2 or frames.shape[1] < 6:
        raise ValueError("Legacy motion frames must be a 2D tensor with at least 6 columns, got shape=", frames.shape, "instead.")
    ...
    if motion_data.has_contacts():
        ...
        if body_contacts.ndim == 3 and body_contacts.shape[0] == 1:
            body_contacts = body_contacts[0]
